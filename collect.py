...
def quote(tk):
    """야후가 직접 계산한 현재가와 전일 종가.

    선물 · 환율 · 코인은 24시간 돌아가서 '하루 봉'의 경계가 한국 시간과
    어긋난다. 일봉으로 전일 대비를 계산하면 한국 오전에 엉뚱한 기준값이
    잡혀 나스닥 선물이 +4% 로 찍히는 식의 오류가 난다.
    그래서 여기서는 야후가 주는 previousClose 를 그대로 쓴다.
    증권 사이트들이 표시하는 기준과 같다.
    """
    try:
...
def collect_globals():
    ...
    for tk, (name, kind, unit) in GLOBALS.items():
        ...
        out.append({
            ...
            'last': round(last, 0 if abs(last) >= 1000 else 2),
            'chg': pct(last, prev),
        })
    return out, errs
